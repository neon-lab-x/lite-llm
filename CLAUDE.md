...
The repo has isolated local and production pretraining flows:
...
- `flow_validation.py` — validates local/production configs stay isolated.
...
| `scripts/production/download_data.py` | HF 源数据 → 本地筛选 raw parquet |
| `scripts/production/tokenize_raw_data.py` | 本地 raw parquet → token shard |
...
- Configuration fields in `train.yaml`, `model.yaml`, or `datasets.yaml`
